...
public class ClientsControllerTests : IClassFixture<BCKashWebApplicationFactory>
{
    ...
    public async Task Create_and_fetch_a_client()
    {
        ...
        var response = await client.PostAsJsonAsync("/api/clients", NewClientRequest("Ada", "Okafor"));
    ...
    public async Task Two_clients_created_back_to_back_get_distinct_sequential_account_numbers()
    {
        ...
        var first = await (await client.PostAsJsonAsync("/api/clients", NewClientRequest("First", "Client"))).Content.ReadFromJsonAsync<ClientResponse>(TestJson.Options);
        var second = await (await client.PostAsJsonAsync("/api/clients", NewClientRequest("Second", "Client"))).Content.ReadFromJsonAsync<ClientResponse>(TestJson.Options);
    ...
    public async Task Update_applies_editable_fields_and_leaves_account_number_and_status_untouched()
    {
        ...
        var created = await (await client.PostAsJsonAsync("/api/clients", NewClientRequest("Bola", "Adeyemi"))).Content.ReadFromJsonAsync<ClientResponse>(TestJson.Options);
    ...
    public async Task Status_transition_only_succeeds_from_its_valid_predecessor(ClientStatus fromStatus, string transition)
    {
        ...
        {
            "activate" => await httpClient.PostAsJsonAsync($"/api/clients/{clientId}/activate", new ActivateClientRequest(null)),
            "deactivate" => await httpClient.PostAsJsonAsync($"/api/clients/{clientId}/deactivate", new ReasonRequest("Integration test reason")),
            "reactivate" => await httpClient.PostAsync($"/api/clients/{clientId}/reactivate", null),
            "decline" => await httpClient.PostAsJsonAsync($"/api/clients/{clientId}/decline", new ReasonRequest("Integration test reason")),
            "close" => await httpClient.PostAsJsonAsync($"/api/clients/{clientId}/close", new ReasonRequest("Integration test reason")),
            _ => throw new ArgumentOutOfRangeException(nameof(transition)),
        };
    ...
    public async Task Deactivate_without_a_reason_is_rejected()
    {
        ...
        var created = await (await httpClient.PostAsJsonAsync("/api/clients", NewClientRequest("Reason", "Required"))).Content.ReadFromJsonAsync<ClientResponse>(TestJson.Options);
        await httpClient.PostAsJsonAsync($"/api/clients/{created!.Id}/activate", new ActivateClientRequest(null));

        var response = await httpClient.PostAsJsonAsync($"/api/clients/{created.Id}/deactivate", new ReasonRequest(""));
    ...
    public async Task Search_filter_and_pagination_narrow_the_client_list()
    {
        ...
        await httpClient.PostAsJsonAsync("/api/clients", NewClientRequest($"Zainab{unique}", "Search", mobile: $"0800{unique}", bvn: $"BVN{unique}"));
        await httpClient.PostAsJsonAsync("/api/clients", NewClientRequest("Someone", "Else"));

        var byName = await httpClient.GetFromJsonAsync<PagedResult<ClientListItemResponse>>($"/api/clients?search=Zainab{unique}", TestJson.Options);
        Assert.Equal(1, byName!.TotalCount);
        Assert.Equal($"Zainab{unique}", byName.Items[0].FirstName);

        var byBvn = await httpClient.GetFromJsonAsync<PagedResult<ClientListItemResponse>>($"/api/clients?bvn=BVN{unique}", TestJson.Options);
        Assert.Equal(1, byBvn!.TotalCount);

        var byMobile = await httpClient.GetFromJsonAsync<PagedResult<ClientListItemResponse>>($"/api/clients?mobile=0800{unique}", TestJson.Options);
        Assert.Equal(1, byMobile!.TotalCount);

        var noMatch = await httpClient.GetFromJsonAsync<PagedResult<ClientListItemResponse>>($"/api/clients?search=NoSuchClient{unique}", TestJson.Options);
    ...
    public async Task Page_size_is_clamped_to_the_configured_maximum()
    {
        ...
        var result = await httpClient.GetFromJsonAsync<PagedResult<ClientListItemResponse>>("/api/clients?pageSize=5000", TestJson.Options);
    ...
    public async Task Reads_are_allowed_without_the_manage_permission_but_writes_are_not()
    {
        ...
        var readResponse = await httpClient.GetAsync("/api/clients");
        Assert.Equal(HttpStatusCode.OK, readResponse.StatusCode);

        var writeResponse = await httpClient.PostAsJsonAsync("/api/clients", NewClientRequest("No", "Permission"));
