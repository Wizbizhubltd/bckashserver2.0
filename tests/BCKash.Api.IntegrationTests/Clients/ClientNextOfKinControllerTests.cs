...
public class ClientNextOfKinControllerTests : IClassFixture<BCKashWebApplicationFactory>
{
    ...
    public async Task Create_list_update_and_delete_a_next_of_kin_record()
    {
        ...
        var createResponse = await client.PostAsJsonAsync($"/api/clients/{clientId}/next-of-kin",
        ...
        var listResponse = await client.GetFromJsonAsync<List<ClientNextOfKinResponse>>($"/api/clients/{clientId}/next-of-kin", TestJson.Options);
        Assert.Single(listResponse!);

        var updateResponse = await client.PutAsJsonAsync($"/api/clients/{clientId}/next-of-kin/{created!.Id}",
        ...
        var deleteResponse = await client.DeleteAsync($"/api/clients/{clientId}/next-of-kin/{created.Id}");
    ...
    public async Task Next_of_kin_for_a_nonexistent_client_return_404()
    {
        ...
        var response = await client.GetAsync("/api/clients/999999/next-of-kin");
