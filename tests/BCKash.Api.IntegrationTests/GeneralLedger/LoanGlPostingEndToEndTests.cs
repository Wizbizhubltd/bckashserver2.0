...
public class LoanGlPostingEndToEndTests : IClassFixture<BCKashWebApplicationFactory>
{
    ...
    public async Task Every_loan_transaction_in_the_servicing_scenario_has_matching_balanced_gl_entries()
    {
        ...
        var disburseResponse = await client.PostAsJsonAsync($"/api/loans/{loanId}/disburse", new DisburseLoanRequest(new DateOnly(2026, 1, 1), 12000m, null));
        Assert.Equal(HttpStatusCode.OK, disburseResponse.StatusCode);

        var repayment1 = await (await client.PostAsJsonAsync($"/api/loans/{loanId}/repayments", new RecordRepaymentRequest(1120m, null, new DateOnly(2026, 1, 28), "On-time")))
            .Content.ReadFromJsonAsync<LoanTransactionResponse>(TestJson.Options);
        var repayment2 = await (await client.PostAsJsonAsync($"/api/loans/{loanId}/repayments", new RecordRepaymentRequest(1120m, null, new DateOnly(2026, 3, 22), "Late")))
            .Content.ReadFromJsonAsync<LoanTransactionResponse>(TestJson.Options);
        ...
        {
            var entries = await client.GetFromJsonAsync<List<GlJournalEntryResponse>>($"/api/gl/journal-entries?glAccountId={portfolio}", TestJson.Options);
        ...
        var allLoanEntries = await client.GetFromJsonAsync<List<GlJournalEntryResponse>>($"/api/gl/journal-entries?glAccountId={portfolio}", TestJson.Options);
        var incomeEntries = await client.GetFromJsonAsync<List<GlJournalEntryResponse>>($"/api/gl/journal-entries?glAccountId={incomeInterest}", TestJson.Options);
        var fundSourceEntries = await client.GetFromJsonAsync<List<GlJournalEntryResponse>>($"/api/gl/journal-entries?glAccountId={fundSource}", TestJson.Options);
