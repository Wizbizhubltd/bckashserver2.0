...
public class LoanWriteOffControllerTests : IClassFixture<BCKashWebApplicationFactory>
{
    ...
    public async Task Write_off_moves_all_outstanding_amounts_out_of_the_active_portfolio()
    {
        ...
        var response = await client.PostAsJsonAsync($"/api/loans/{loanId}/write-off", new WriteOffLoanRequest("Client unreachable, deemed unrecoverable", new DateOnly(2026, 6, 1)));
    ...
    public async Task Recovery_after_write_off_is_recorded_as_its_own_transaction()
    {
        ...
        await client.PostAsJsonAsync($"/api/loans/{loanId}/write-off", new WriteOffLoanRequest("Unrecoverable", null));

        var response = await client.PostAsJsonAsync($"/api/loans/{loanId}/record-recovery", new RecordRecoveryRequest(2000m, new DateOnly(2026, 8, 1), "Partial recovery via collections agency"));
    ...
    public async Task Write_off_requires_the_loan_to_be_disbursed()
    {
        ...
        await client.PostAsJsonAsync($"/api/loans/{loanId}/write-off", new WriteOffLoanRequest("First write-off", null));

        var secondWriteOff = await client.PostAsJsonAsync($"/api/loans/{loanId}/write-off", new WriteOffLoanRequest("Second attempt", null));
    ...
    public async Task Recovery_requires_the_loan_to_already_be_written_off()
    {
        ...
        var response = await client.PostAsJsonAsync($"/api/loans/{loanId}/record-recovery", new RecordRecoveryRequest(1000m, null, null));
